...
void Nesterov::initLayer(vector<Layer*>& network) {
	int size = static_cast<int>(network.size());

	for (int i = 0; i < size; ++i) {
		LayerType type = network[i]->getLayerType();
		if (type == LayerType::Affine || type == LayerType::BatchNorm) {
...
void Nesterov::initParams(float learning_rate, float _momentum, float not_use1) {
	learningRate = learning_rate;
	decayLearningRate = learning_rate;
	momentum = _momentum;
}

void Nesterov::update(vector<Layer*>& network) {
	...
	for (iter = params.begin(); iter != params.end(); iter++) {
		...
		for (int i = 0; i < size; ++i) {
			v[i] *= momentum;
			v[i] -= (*grad[i] * decayLearningRate);// learningRate);
			*param[i] += (v[i] * (momentum * momentum));
			*param[i] -= *grad[i] * ((1.0f + momentum) * decayLearningRate);// learningRate);
		}
	}
}